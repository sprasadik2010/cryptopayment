...
export default function SecondLevelClubShare({onUpdate}) {
    ...
    const fetchProfitData = async () => {
      ...
      try {
        const [profitRes, userCountRes] = await Promise.all([
          ...
          fetch(`${import.meta.env.VITE_CRYPTO_PAYMENT_API_BASE_URL}/second-level-members`)
        ]);
        ...
        onUpdate(totalPayout,"secondlevelclub");
      } catch (err) {
        console.error("Error fetching second level club data:", err);
      }
    };
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Second Level Club Payouts</h2>
